...
namespace DX
{
	inline void ThrowIfFailed(HRESULT hr)
	{
		if (FAILED(hr))
		{
			...
			////	Stack trace :
			////[External Code]
			////WpDxBandSampleNative.dll!DX::ThrowIfFailed(HRESULT hr) Line 19
			////	WpDxBandSampleNative.dll!WpDxBandSampleNative::DirectXPanelBase::CreateDeviceResources() Line 120
			////	WpDxBandSampleNative.dll!WpDxBandSampleNative::D3DPanel::[WpDxBandSampleNative::DirectXPanelBase]::CreateDeviceResources() Line 198
			//
			// Then better run. Nah, just use RELEASE mode or comment out << creationFlags |= D3D11_CREATE_DEVICE_DEBUG; >> in DirectXPanelBase::CreateDeviceResources()
		}
	}
