...
using namespace WpDxBandSampleNative;
using namespace DX;
...
void D3DPanel::Render()
{
	...
	// Rotate and Scale
	auto rotate			     = XMMatrixRotationRollPitchYaw(Pitch, Yaw, Roll);
	auto scale				 = XMMatrixScaling(Scale, Scale, Scale);
	auto modelTransformation = XMMatrixMultiply(scale, rotate);

	// Prepare to pass the view matrix, and updated model matrix, to the shader
	XMStoreFloat4x4(&m_constantBufferData.view, XMMatrixTranspose(XMMatrixLookAtRH(eye, at, up)));
	XMStoreFloat4x4(&m_constantBufferData.model, XMMatrixTranspose(modelTransformation));
	...
	m_d3dContext->RSGetState(&prevRasterState);
	m_d3dContext->RSSetState(m_rasterStateWireFrame.Get());
	m_d3dContext->PSSetShader(m_pixelShaderSolidColor.Get(), nullptr, 0);
	m_d3dContext->DrawIndexed(m_indexCount, 0, 0);
	m_d3dContext->RSSetState(prevRasterState.Get());
...
void D3DPanel::CreateDeviceResources()
{
	...
	m_d3dDevice->CreateRasterizerState(&wireframeRasterDesc, &m_rasterStateWireFrame);

	// Asynchronously load vertex shader and create input layout.
	auto loadVSTask = DX::ReadDataAsync(L"WpDxBandSampleNative\\SimpleVertexShader.cso");
	...
	auto loadPSTask = DX::ReadDataAsync(L"WpDxBandSampleNative\\SimplePixelShader.cso");
	...
	auto loadPSTask2 = DX::ReadDataAsync(L"WpDxBandSampleNative\\WhitePixelShader.cso");
	auto createPSTask2 = loadPSTask2.then([this](const std::vector<byte>& fileData) {
		ThrowIfFailed(
			...
			fileData.size(),
			nullptr,
			&m_pixelShaderSolidColor
			)
			);
	});

	// Once both shaders are loaded, create the mesh.
	auto createCubeTask = (createPSTask2 && createPSTask && createVSTask).then([this]() {
